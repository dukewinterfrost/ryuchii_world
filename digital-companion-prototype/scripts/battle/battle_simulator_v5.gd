...
const ContentSchema = preload("res://scripts/battle/mob_content.gd")
...
static func create_roster_session(battle_id: String, battle_seed: int, roster: Array, arena: Dictionary, max_ticks: int, visuals: Dictionary, initial_supplies: Dictionary, pinned_items: Dictionary, encounter_id: String, arena_sha: String, config: Dictionary) -> Dictionary:
	...
	for input: Variant in roster:
		...
		var species: Dictionary = config.species.get(fighter.species_id, {})
		if species.is_empty() or fighter.basic_move != species.basic_move or fighter.special.id != species.special or fighter.stage != species.stage:
			return _error("fighter does not match pinned species registry")
		...
		actors[id] = {"fighter_id": id, "team_id": fighter.team_id, "controller_id": fighter.controller_id,
			...
			"target_id": "", "focus_target_id": "", "support_target_id": "", "effects": {}, "opening_pending": bool(normalized.natures[fighter.nature].opening_tackle) and not fighter.species_id.begins_with("slime") and not fighter.species_id.begins_with("fairy")}
	if teams.size() < 2:
...
static func preflight_commands(session: Dictionary, commands: Array) -> Dictionary:
	if session.get("simulation_version") != SIMULATION_VERSION:
		return _error("commands require battle-v5")
...
static func _preflight(command: Variant, session: Dictionary, tick: int) -> Dictionary:
	...
	match kind:
		"focus_target":
			if not command.get("target_id") is String or not session.actors.has(command.target_id):
				return _error("unknown focus target")
			var target: Dictionary = session.actors[command.target_id]
			if int(target.hp) <= 0 or target.team_id == actor.team_id:
				return _error("focus requires a living hostile target")
			accepted.target_id = command.target_id
		"order":
...
static func _apply_command(session: Dictionary, command: Dictionary) -> void:
	...
	match command.kind:
		"focus_target":
			actor.focus_target_id = command.target_id
			_emit(session, "focus_changed", command.fighter_id, command.target_id, "Focus target changed.")
		"order":
...
static func _update_actor(session: Dictionary, id: String, threat: Dictionary, rng: BattleRng) -> void:
	...
	var focus := String(actor.get("focus_target_id", ""))
	if session.actors.has(focus) and int(session.actors[focus].hp) > 0 and session.actors[focus].team_id != actor.team_id:
		actor.target_id = focus
	else:
		actor.focus_target_id = ""
	...
	if not threat.is_empty() and tick >= int(actor.ai_defense_ready_tick):
		...
		if not actor.reacted_threats.has(key) and tick >= int(actor.seen_threats[key]):
			...
			return
	for support_id: String in _automatic_moves(snapshot):
		var support: Dictionary = snapshot.moves[support_id]
		if support.kind in ["heal", "barrier"] and _available(actor, support, tick) and not _support_target(session, id, support).is_empty():
			_commit_move(session, id, support)
			return
	...
	if tick >= int(actor.next_decision):
		...
		for move_id: String in _automatic_moves(snapshot):
			...
			if move.kind not in ["heal", "barrier"] and _available(actor, move, tick) and _move_in_range(session, id, move) and int(move.auto_weight) > 0:
				candidates.append(move)
				total_weight += int(move.auto_weight)
		var fallback: Dictionary = snapshot.moves[snapshot.basic_move]
...
static func _tactical_move(session: Dictionary, id: String) -> void:
	...
	var melee_range := int(snapshot.moves[snapshot.basic_move].range)
...
static func _commit_move(session: Dictionary, id: String, move: Dictionary) -> void:
	...
	actor.support_target_id = _support_target(session, id, move) if move.kind in ["heal", "barrier"] else ""
	_start_action(session, id, "special_attack" if move.kind in ["projectile", "heal", "barrier"] else ("rush_attack" if move.kind == "rush" else "basic_attack"), int(move.duration), int(move.windup), int(move.active))
...
static func _observable_threat(session: Dictionary, id: String) -> Dictionary:
	...
	for enemy_id: String in session.fighter_order:
		...
		if enemy.team_id == actor.team_id or int(enemy.hp) <= 0 or enemy.action not in ATTACKS or enemy.current_move.is_empty() or enemy.current_move.get("kind") in ["heal", "barrier"]:
			continue
...
static func _advance_attack(session: Dictionary, id: String, hits: Array) -> void:
	...
	if not bool(actor.released):
		...
		if move.kind in ["heal", "barrier"]:
			_release_support(session, id, move)
...
static func validate_snapshot(snapshot: Dictionary) -> String:
	...
	if not snapshot.get("basic_move") is String or snapshot.nature not in SUPPORTED_NATURES:
		return "unknown species or nature"
	if snapshot.stage != {"botamon": "Baby", "koromon": "In-Training", "agumon": "Rookie"}.get(snapshot.species_id, "Rookie"):
	...
	if not snapshot.get("special") is Dictionary or not snapshot.special.get("id") is String:
		return "invalid species special"
	var fallback := String(snapshot.basic_move)
	if not snapshot.moves.has_all([fallback, String(snapshot.special.id), "opening_tackle"]):
	...
	for id: Variant in snapshot.moves:
		...
		if move.get("id") != id or not move.get("name") is String or move.get("kind") not in ["melee", "projectile", "rush", "heal", "barrier"]:
...
static func _support_target(session: Dictionary, id: String, move: Dictionary) -> String:
	var actor: Dictionary = session.actors[id]
	var selected := ""
	var best_hp := 1
	var best_maximum := 0
	for candidate: String in session.fighter_order:
		var ally: Dictionary = session.actors[candidate]
		if ally.team_id != actor.team_id or int(ally.hp) <= 0:
			continue
		if move.target == "self" and candidate != id:
			continue
		if move.target == "ally_slime" and not String(_snapshot(session, candidate).species_id).begins_with("slime"):
			continue
		if _length(_subtract(ally.pos, actor.pos)) > int(move.range) or Arena.obstruction_fraction(session.arena, actor.pos, ally.pos, 0, "sight") >= 0:
			continue
		var maximum := int(_snapshot(session, candidate).stats.hp)
		if move.kind == "heal" and int(ally.hp) * 1000 >= int(move.trigger_percent) * maximum:
			continue
		if move.kind == "barrier" and ally.effects.has("barrier"):
			continue
		if selected.is_empty() or int(ally.hp) * best_maximum < best_hp * maximum:
			selected = candidate
			best_hp = int(ally.hp)
...
static func _release_support(session: Dictionary, id: String, move: Dictionary) -> void:
	var actor: Dictionary = session.actors[id]
	var target_id := String(actor.support_target_id)
	if not session.actors.has(target_id):
		return
	var target: Dictionary = session.actors[target_id]
	if int(target.hp) <= 0 or target.team_id != actor.team_id or _length(_subtract(target.pos, actor.pos)) > int(move.range) or Arena.obstruction_fraction(session.arena, actor.pos, target.pos, 0, "sight") >= 0:
		_emit(session, "support_missed", id, target_id, "Support target unavailable.")
		return
	if move.kind == "heal":
		var maximum := int(_snapshot(session, target_id).stats.hp)
		var amount := mini(maximum - int(target.hp), maxi(1, maximum * int(move.heal_percent) / 1000))
		target.hp = int(target.hp) + amount
		_emit(session, "healed", id, target_id, "%s heals %s for %d HP." % [_snapshot(session, id).display_name, _snapshot(session, target_id).display_name, amount], {"restored": amount, "move_id": move.id})
	elif not target.effects.has("barrier"):
		target.effects.barrier = {"amount": int(move.barrier_amount), "expires_tick": int(session.tick) + int(move.effect_duration)}
		_emit(session, "warded", id, target_id, "Protective ward applied.", {"move_id": move.id})
