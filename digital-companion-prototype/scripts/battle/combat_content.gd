...
const SCHEMA_VERSION := "combat-content-v1"
const TICKS_PER_SECOND := 30
const SCALE := 1000
const SPECIES := ["botamon", "koromon", "agumon"]
...
static func validate_snapshot(config: Dictionary) -> String:
	...
	for id: Variant in config.moves:
		...
		if move.get("id") != id or not _short_text(move.get("name")) or move.get("kind") not in ["melee", "projectile", "rush"]:
			return "moves.csv row 1 column kind: invalid pinned move '%s'" % id
		if not move.get("species") is Array or move.species.is_empty() or move.species.size() > SPECIES.size():
			return "moves.csv row 1 column species: invalid species for '%s'" % id
		var seen := {}
		for species: Variant in move.species:
			if species not in SPECIES or seen.has(species):
		...
		if move.get("animation") not in ["basic_attack", "special_attack"] or move.get("effect") not in ["impact", "fire", "bubble", "rush"]:
...
static func _normalize_move(row: Dictionary, errors: Array[String]) -> Dictionary:
	...
	if kind not in ["melee", "projectile", "rush"]:
	...
	for value: String in String(row.species).split("|"):
		...
		if id not in SPECIES or id in species:
	...
	if row.effect not in ["impact", "fire", "bubble", "rush"]:
...
static func _validate_required_rows(config: Dictionary, errors: Array[String]) -> void:
	for species: String in SPECIES:
		for id: String in [FALLBACKS[species], SPECIALS[species], "opening_tackle"]:
		...
		if config.moves.has(FALLBACKS[species]):
			var basic: Dictionary = config.moves[FALLBACKS[species]]
			if basic.get("kind") != "melee" or basic.get("mp_cost") != 0 or basic.get("equippable") != false:
				errors.append("moves.csv row 1 column mp_cost: '%s' must remain a free, non-equippable melee fallback" % FALLBACKS[species])
