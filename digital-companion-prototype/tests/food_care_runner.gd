...
func run() -> void:
	...
	check(CareRules.state_is_valid(upgraded) and upgraded.identity == old.identity and upgraded.inventory == old.inventory and upgraded.care.poop_slots == old.care.poop_slots, "v5 migration preserves identity, supplies and waste")
